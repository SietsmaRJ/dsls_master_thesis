...
os.chdir("/home/sietsmarj/Documents/School/Master_DSLS/Final_Thesis/Initial_Data_exploration")

# Load in training data
var_train_res = pd.read_csv("train_results.txt", sep='\t', header=0)
var_train_res.columns = ["chr","pos","ref","alt","gene","csq1","cadd","capice","pred","combpred"]
...
var_test.columns = ["chr","pos","ref","alt","maf","csq3","label","revel","clinpred","sift","provean","cadd","fathmm","capice","ponp2"]
...
capice = capice_train[["chr","pos","ref","alt","src","label","capice"]].append(var_test[["chr","pos","ref","alt","src","label","capice"]])
del(capice_train, var_test)

vkgl_labels = pd.read_csv("VKGL_14nov2019.txt", sep='\t', header=0)
vkgl_labels.columns = ["variant","chr","pos","ref","alt","cdna","protein","transcript","hgvs","gene","label","nrlabs"]
...
overlap_capice = pd.merge(vkgl, capice, on=['chr','pos','ref','alt'])[['chr','pos','ref','alt']]
temp_not_overlap = pd.merge(vkgl, overlap_capice, on=['chr','pos','ref','alt'], how='outer', indicator=True)
vkgl1NIC = temp_not_overlap[temp_not_overlap['_merge'] == 'left_only']
vkgl1IC = pd.merge(vkgl, overlap_capice, on=['chr','pos','ref','alt'])

del(vkgl, overlap_capice, temp_not_overlap)

all = capice[["chr","pos","ref","alt","src","label","capice"]].append(
    vkgl1NIC[["chr","pos","ref","alt","src","label","capice"]]
)
del(capice, vkgl1NIC, vkgl1IC)
